...
def main(
    ...
    assert set(gt.keys()) == set(pred.keys()), 	    "Some of the dataset keys are missing in the preditions: " + str((gt.keys()) ^ set(pred.keys()))

    results = dict()
    for dataset in gt.keys():
        if strict:
            assert set(gt[dataset].keys()) == set(pred[dataset].keys()), 			   "Some of the images are missing in the predictions: " + str(set(gt[dataset].keys()) ^ set(pred[dataset].keys()))
